...
```
Subsetting the dataset to only include policies with positive claims, reduces the dataset from `r n.rows.total` rows to only `r n.rows.with.claims` rows.
...
Before estimating/training a model we split the data in test and training set to be able to compute validation metics later on.
...
```
We reserve `r sprintf("%.0f%%", test.pct*100)` of the data set for testing, so the training set conatians `r nrow(train.data)` observations and the remaining `r nrow(test.data)` observations are used for testing.
